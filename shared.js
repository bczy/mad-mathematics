  ...
  try {
    ...
    if (element) {
      ...
      if (!headers || !headers.classList.contains('hs-headers')) {
        ...
        headers.innerHTML =
          '<div class="hs-col-rank">Rang</div><div class="hs-col-name">Nom</div><div class="hs-col-score">Score / Temps</div>';
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (s) {
        ...
        const scoreDisplay =
          level === 'super-multi' ? `${s.score} pts` : `${s.score}/15`;
        const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : '';
        const rank = i + 1;
        rows.push(
          `<li class="highscore-item"><span class="hs-left"><span class="hs-rank">${rank}.</span> ${medal} ${s.name}</span><span class="hs-right">${scoreDisplay} <small class="hs-time">(${timeStr})</small></span></li>`
        );
      } else {
        const rank = i + 1;
        rows.push(
          `<li class="highscore-item empty"><span class="hs-left"><span class="hs-rank">${rank}.</span> Aucun score</span></li>`
        );
      }
    }
    ...
    if (element) {
      element.innerHTML =
        '<li class="highscore-item empty">Erreur de chargement</li>';
    }
  }
}
