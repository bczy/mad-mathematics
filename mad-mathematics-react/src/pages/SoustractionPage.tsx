/**
 * SoustractionPage - Mad Mathematics
 * Subtraction game page with per-question timer and 4 difficulty levels
 * Ensures num1 >= num2 for positive results
 */
...
import { SOUSTRACTION_DIFFICULTIES, createHighscoreKey } from '../types';
import type { Difficulty, Answer, Question } from '../types';

/**
 * Soustraction game page component
 * Implements the full game flow for practicing subtraction
 * Uses per-question time limits instead of total game time
 */
export function SoustractionPage() {
  ...
  // Game logic hook - subtraction with positive results only
  ...
  useEffect(() => {
    if (status === 'results' && difficulty && !highscoreSavedRef.current) {
      ...
      const storageKey = createHighscoreKey('soustraction', difficulty.id);
      ...
      <Card className="max-w-md w-full">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-amber-400 mb-2">
            ➖ École de Magie des Soustractions ✨
          </h1>
          <p className="text-purple-200">
            Entraîne-toi à soustraire comme un magicien !
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          <DifficultySelector
            difficulties={SOUSTRACTION_DIFFICULTIES}
            onSelect={onStart}
          />
        </div>
...
export default SoustractionPage;
